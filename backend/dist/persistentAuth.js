...
async function load() {
    ...
    try {
        ...
        if (parsed && typeof parsed === "object" && parsed.version === 1 && parsed.sites) {
            cache = { version: 1, sites: parsed.sites };
            return cache;
        }
    }
    catch {
...
export async function getStoredSiteAuth(siteId) {
...
export async function upsertStoredSiteAuth(entry) {
...
export async function touchStoredSiteAuth(siteId) {
    const id = String(siteId || "").trim();
    if (!id)
        return;
    const store = await load();
...
export async function removeStoredSiteAuth(siteId) {
    const id = String(siteId || "").trim();
    if (!id)
        return;
    const store = await load();
