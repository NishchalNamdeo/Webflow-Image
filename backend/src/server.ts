...
function getAccessTokenForSite(req: Request, siteId: string): string {
  ...
  const token = String(entry?.accessToken || (req.session as any)?.accessToken || "").trim();
  return token;
}
  ...
  asyncRoute(async (req: Request, res: Response) => {
    ...
    if (!rec) {
      return res.status(403).json({
        message:
          "This site is not authorized on the server yet. Please authorize once (open /auth) and then try again.",
      });
    }
    ...
    const resolvedSiteId = String((user as any)?.siteId || "").trim();
    if (resolvedSiteId && resolvedSiteId !== siteId) {
      return res.status(401).json({ message: "ID token does not match this site" });
    }

    // Restore session
    (req.session as any).accessToken = rec.accessToken;
    (req.session as any).scopes = rec.scopes;
